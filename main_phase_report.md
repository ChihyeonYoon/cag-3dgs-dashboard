...
## 6. 결론 및 향후 보완 과제
* 본 PoC를 통해 **Multi-Task DUNet + Focal Loss**의 고품질 해부학적 분지 예측과 **3DGS 미분 가능한 렌더링 최적화** 기법을 연계하여, 3D Ground Truth 데이터 없이 오직 임상 2D Angiography 영상들만으로 3차원 관상동맥 볼륨과 직경 상태를 완벽하게 역추적할 수 있는 독자적이고 강력한 기하-뉴럴 하이브리드 파이프라인을 완성했습니다.
* **향후 연구 방향**: 심장의 끊임없는 박동 상태를 보정하기 위해 연속적인 시간 차원을 매핑하는 **Dynamic 3DGS (4D-CAG)** 및 다중 뷰 촬영 시의 미세한 동기화 오차와 C-Arm 카메라의 캘리브레이션 미세 오차를 함께 조율하는 **공동 카메라 포즈 최적화 (Joint Pose-Gaussian Optimization)** 단계를 파이프라인에 통합함으로써, 상용 3D CT 복원에 필적하는 실시간 혈관 3D 가우시안 복원 솔루션으로 고도화가 가능할 것으로 기대됩니다.
