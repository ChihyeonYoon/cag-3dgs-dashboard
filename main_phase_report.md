...
논문의 다중 클래스 베이스라인인 YOLOv8x와 당사가 제안한 Multi-Task DUNet(손실 함수 조건별) 간의 2D 세그멘테이션 Dice 성능 직접 대조 결과입니다.

| 모델 출처 | 모델명 | 손실 함수 조건 | 평가 기준 | Mean Dice |
| :--- | :--- | :--- | :---: | :---: |
| **당사 제안 모델** | **Multi-Task DUNet (Focal)** | **클래스 가중 Focal Loss** | **전체 혈관 통합 (Merged Vessel)** | **76.55% (0.7655)** |
...
| :--- | :--- | :--- | :---: | :---: | :---: |
| **당사 제안 모델** | **Multi-Task DUNet (BCE)** | DCA1 Test (34장) | 멀티태스크 내 이진 분할 | **77.95% (0.7795)** | **64.06% (0.6406)** |
...
* **전 모델 논문 대비 우위 달성**: 당사가 재현 및 구현한 모든 모델군(싱글/멀티태스크 전량)이 논문 제시 베이스라인(75.00%)을 크게 웃돕니다.
...
## 6. 결론 및 임상적 한계와 향후 보완 과제

* **결론 및 의의**: 본 PoC를 통해 **Multi-Task DUNet + Focal Loss** 기반의 세부 분지 분류와 **3D 가우시안 스플래팅(3DGS)** 최적화 루프를 결합하여, 3D Ground Truth(CT 등) 데이터가 부재한 상황에서도 2D Angiography 투영 영상들 간의 기하학적 일관성만을 이용해 관상동맥의 3차원 주행 궤적 및 내강 직경 변화 추세를 근사적으로 복원하는 기하-뉴럴 하이브리드 파이프라인의 타당성을 입증하였습니다.
* **임상적/기술적 한계점**:
  1. **정보의 제한성**: CT와 달리 조영제가 채워진 혈관 내강(Lumen) 정보만을 투영하므로 혈관벽 두께나 동맥경화반(Plaque)의 세부 성상까지 파악하는 데는 한계가 있습니다.
  2. **에피폴라 모호성**: 투영 뷰가 2개(Biplane)로 극히 제한적일 경우, 분지가 중첩되거나 카메라 평면과 평행하게 달리는 영역에서는 기하학적 복원의 불확실성(Depth Ambiguity)이 여전히 존재합니다.
  3. **시간적 비동기성**: 심장 박동 및 호흡으로 인한 혈관의 동적 움직임과 촬영 프레임 간의 미세한 비동기화는 복원 정밀도를 저하시키는 주요 요인입니다.
* **향후 연구 방향 (보완 과제)**:
  * **Dynamic 3DGS (4D-CAG) 도입**: 시간 차원($t$)을 축으로 설정하여 심장 주기(Cardiac Cycle)에 따른 혈관의 변형을 추적하는 동적 복원 기법을 통합합니다.
  * **Joint Pose-Gaussian Optimization**: C-Arm C-arm 카메라의 미세 캘리브레이션 오차와 동기화 오류를 렌더링 최적화 과정에서 공동 학습하여 기하학적 정밀도를 개선합니다.
  * **시술 보조 시각화 시스템으로의 발전**: 본 파이프라인을 고도화하여 CT 촬영이 어려운 환자군이나 중재시술(PCI) 중 C-Arm 영상을 활용해 실시간으로 3차원 혈관 구조 및 협착 부위의 직경 변화 추세를 직관적으로 파악할 수 있는 보조적 시각화 도구로 활용할 수 있을 것으로 기대됩니다.
